...
module.exports = class GitReader {
      ...
      if (isRepo) {
        git.listRemote(['-q', '--refs']).then(result => {
          if (result) {
            ...
            this.connected = true;
            git.log(['origin/master', '-1', '--pretty=format:"%h"']).then((masterResult) => {
              git.log(['-1', '--pretty=format:"%h"']).then((localResult) => {
                if (localResult.latest.hash === masterResult.latest.hash) {
                  ...
                  console.log(
                    `Git: local has a different version as origin/master ${localResult.latest.hash}/${masterResult.latest.hash}`);
                  this.upToDate = false;
                }
              });
            },
  ...
  checkRevisionAndSendResult (sendPosResult, sendNegResult) {
    ...
    git.log(['origin/master', '-1', '--pretty=format:"%h"']).then((masterResult) => {
      return git.log(['-1', '--pretty=format:"%h"']).then((localResult) => {
        if (localResult.latest.hash === masterResult.latest.hash) {
          this.upToDate = true;
